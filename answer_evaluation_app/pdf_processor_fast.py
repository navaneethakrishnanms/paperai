...
class PDFProcessor:
    def __init__(self, use_deepseek=False):  # Changed to False by default for speed
        ...
        print("⚡ Fast PDF Processor initialized (using PyPDF2 text extraction)")
    
    def extract_text_from_pdf(self, pdf_path):
        """Extract text from PDF using PyPDF2 - FAST METHOD"""
        try:
            text = ""
    ...
    def extract_questions_with_marks(self, pdf_path):
        """
        Extract questions from question paper with their marks - FAST VERSION
        """
        print(f"⚡ Fast extraction starting: {os.path.basename(pdf_path)}")
        ...
        if not text.strip() or len(text) < 50:
            print("📄 Question paper appears to be scanned, using OCR...")
    ...
    def extract_questions(self, pdf_path):
        """Backward-compatible wrapper for question extraction"""
        return self.extract_questions_with_marks(pdf_path)

    def extract_answers(self, pdf_path):
        """Extract answers from answer key PDF - FAST VERSION"""
        print(f"⚡ Fast extraction starting: {os.path.basename(pdf_path)}")
        ...
        # If text extraction failed or returned very little, try OCR
        if not text.strip() or len(text) < 50:
            print("📄 Answer key appears to be scanned, using OCR...")
            text = self.ocr_pdf(pdf_path)
    ...
    def extract_student_answers(self, pdf_source):
        """
        ⚡ FAST VERSION - Extract student answers using SAME METHOD as question/answer key
        Accepts either a PDF path or pre-extracted text content (e.g., from DeepSeek OCR)
        """
        try:
            text = ""
            ...
            if isinstance(pdf_source, str) and os.path.exists(pdf_source):
                ...
                print(f"⚡ FAST extraction - Student answers from file: {source_desc}")

                # Use the SAME fast extraction method
                text = self.extract_text_from_pdf(pdf_path)

                # If text extraction failed, try OCR (but still fast)
                if not text.strip() or len(text) < 50:
                    print("📄 Student paper appears to be scanned, using OCR...")
                    text = self.ocr_pdf(pdf_path)
            elif isinstance(pdf_source, str):
                text = pdf_source
                print("⚡ FAST extraction - Student answers from OCR text input")
            else:
            ...
            print(f"📝 First 300 chars: {clean_text[:300]}")
            
            student_answers = []
            
            print("🔍 Searching for student answers with multiple patterns...")
            
            # Strategy 1: Number followed by period and content (like your document)
            ...
            for match in matches1:
                try:
                    ...
                    if answer_text and len(answer_text) > 10:
                        ...
                        print(f"  ✓ Pattern 1: Found answer {q_num} ({len(answer_text)} chars)")
                except Exception as e:
                    print(f"  ✗ Error in pattern 1: {e}")
                    continue
            
            # Strategy 2: Traditional answer patterns
            traditional_patterns = [
            ...
            for idx, pattern in enumerate(traditional_patterns, 2):
                matches = re.finditer(pattern, clean_text, re.IGNORECASE | re.DOTALL)
                for match in matches:
                    try:
                        q_num = int(match.group(1))
                        ...
                        if answer_text and len(answer_text) > 10:
                            ...
                            print(f"  ✓ Pattern {idx}: Found answer {q_num} ({len(answer_text)} chars)")
                    except:
                        continue
            ...
            print(f"✅ Extracted {len(student_answers)} student answers (FAST)")
            
            # Show details of what we found
            if student_answers:
                for ans in student_answers:
                    print(f"   Q{ans['question_number']}: {ans['student_answer'][:80]}...")
            else:
                print("⚠️ No student answers found!")
    ...
    def generate_result_pdf(self, evaluation_result, output_path):
        ...
        try:
            ...
            footer_text = "Generated by AI-Powered Answer Evaluation System (Fast Mode)"
    ...
    def _generate_text_report(self, evaluation_result, output_path):
        ...
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                ...
                f.write("="*60 + "\n")
                f.write("Generated by AI-Powered Answer Evaluation System (Fast Mode)\n")
